...
def doc(i):
    ...
    for repo in list_of_repos:
        parsed_artifact[1] = ('', repo) if utils.is_cm_uid(
            repo) else (repo, '')
    ...
    for artifact in sorted(lst, key=lambda x: (-x.meta.get('sort', 0), x.meta.get('alias', ''))):
        ...
        if repo_alias == 'internal':
            ...
            url_repo = 'https://github.com/' + \
                repo_alias.replace('@', '/')+'/tree/master'
...
#                md.append('\n')
        ...
        md.append(
            '* GitHub repository with CM automations: *cm pull [{}]({})*'.format(repo_alias, url_repo))
        ...
        for line in module_py_list:
            if line.startswith('    def '):
                ...
                if not action.startswith('_'):
                    x = '  * cm **'+action+'** '+alias + \
                        '   &nbsp;&nbsp;&nbsp;*( [See CM API]({}) )*'.format(
                            module_url+'#L'+str(line_number))
                    md.append(x)

                    y = '[add flags (dict keys) from this API]({})'.format(
                        module_url+'#L'+str(line_number))
                    y2 = '[add keys from this API]({})'.format(
                        module_url+'#L'+str(line_number))
                    z = '({})'.format(y)
                    ...
                    md_script_readme.append('')
                    md_script_readme.append(
                        '  * CM CLI: ```cm '+action+' '+alias+'``` '+z)
                    md_script_readme.append(
                        '  * CM CLI with UID: ```cm '+action+' '+alias+','+uid+'``` '+z)
