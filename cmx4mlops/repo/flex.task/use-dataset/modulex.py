...
def run(i):
    ...
    if path == '':
        ...
        r = cmind.x({'automation':self_meta['use']['flex.cfg'],
                     ...
                     'artifact':'mlperf'})
        if r['return'] == 0:
            path = r['meta'].get('storage_for_datasets', '')


    if verbose:
        x = dataset_tags if dataset_tags != '' else name
        print ('')
        print (f'INFO Use dataset "{x}"')

    r = cmind.x({'automation': self_meta['use']['flex.common'],
                 'action': 'select_artifact',
                 'selected_text': 'Select dataset',
                 'selected_automation': self_meta['use']['flex.dataset'],
                 'selected_artifact': name,
                 'selected_tags': dataset_tags,
                 'quiet': quiet,
                 'control':{'out':out}})

    if r['return']>0: return cmind.embed_error(r)

    dataset_object = r['selected_artifact_object']

    dataset_meta = dataset_object.meta
    dataset_path = dataset_object.path

    dataset_artifact_name = dataset_meta['alias']
    dataset_artifact_uid = dataset_meta['uid']

    ###############################################################
    # Check deps
    deps = dataset_meta.get('global_deps', [])
    ...
    r = utils.load_module(cmind, dataset_path, 'modulex.py')
    if r['return'] >0: return cmind.embed_error(r)
    ...
    dataset_tags = dataset_meta['tags']
    ...
    rr = {'return':0,
          ...
          '_cmx_dataset_alias': dataset_artifact_name,
          '_cmx_dataset_uid': dataset_artifact_uid,
          '_cmx_dataset_path': dataset_path,
          '_cmx_dataset_meta': dataset_meta
         }

    if sub_module_obj != None and hasattr(sub_module_obj, 'install'):
        cmind.log(f"use-dataset: call 'install' in {sub_module_path}", "info")

        r = sub_module_obj.install({'automation_flex_task_use_dataset':self_meta['use']['flex.dataset'],
                                    'cmind': cmind,
                                    'state': state,
                                    'misc': misc,
                                    'env': env,
