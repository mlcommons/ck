...
#### CM CLI
`cm run script --tags=get,src,source,training,training-src,training-source,mlperf,mlcommons(,variations from below) (flags from below)`

*or*

`cm run script "get src source training training-src training-source mlperf mlcommons (variations from below)" (flags from below)`
