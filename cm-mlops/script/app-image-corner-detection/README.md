...
`cm run script --tags=app,image,corner-detection(,variations from below) (flags from below)`

*or*

`cm run script "app image corner-detection (variations from below)" (flags from below)`
...
```cm run script --tags=gui --script="app,image,corner-detection"```
