...
#RUN cm run script --tags=get,ml-model,object-detection,resnext50,fp32,_onnx
...
RUN cm run script --tags=app,mlperf,inference,generic,_retinanet,_onnxruntime,_cpu,_python --adr.compiler.tags=gcc --adr.inference-src.tags=_octoml --fake_run 
RUN cm run script --tags=app,mlperf,inference,generic,_retinanet,_onnxruntime,_cpu,_python --adr.compiler.tags=gcc --adr.inference-src.tags=_octoml 
