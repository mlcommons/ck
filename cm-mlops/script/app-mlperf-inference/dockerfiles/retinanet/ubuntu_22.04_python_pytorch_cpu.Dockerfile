...
#RUN cm run script --tags=get-ml-model,retinanet,_pytorch
#RUN cm run script --tags=get,dataset,preprocessed,openimages

# Run CM workflow for MLPerf inference
RUN cm run script --tags=app,mlperf,inference,generic,reference,_retinanet,_pytorch,_cpu,_python --adr.compiler.tags=gcc --adr.inference-src.tags=_octoml --fake_run
RUN cm run script --tags=app,mlperf,inference,generic,reference,_retinanet,_pytorch,_cpu,_python --adr.compiler.tags=gcc --adr.inference-src.tags=_octoml
