...
#RUN cm run script --tags=get,generic-python-lib,_tensorflow
...
#RUN cm run script --tags=get,dataset,image-classification,imagenet,preprocessed,_NHWC
#RUN cm run script --tags=get,dataset-aux,image-classification,imagenet-aux
#RUN cm run script --tags=get,ml-model,image-classification,resnet50,_tensorflow
...
RUN cm run script --tags=app,mlperf,inference,generic,_resnet50,_tensorflow,_cpu,_python --adr.compiler.tags=gcc --adr.inference-src.tags=_octoml --fake_run 
RUN cm run script --tags=app,mlperf,inference,generic,_resnet50,_tensorflow,_cpu,_python --adr.compiler.tags=gcc --adr.inference-src.tags=_octoml 
