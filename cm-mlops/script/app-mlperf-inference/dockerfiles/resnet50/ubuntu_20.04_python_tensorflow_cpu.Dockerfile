...
#RUN cm run script --tags=get-ml-model,resnet50,_tensorflow
#RUN cm run script --tags=get,dataset,preprocessed,imagenet

# Run CM workflow for MLPerf inference
RUN cm run script --tags=app,mlperf,inference,generic,reference,_resnet50,_tensorflow,_cpu,_python --adr.compiler.tags=gcc --adr.inference-src.tags=_octoml --fake_run
RUN cm run script --tags=app,mlperf,inference,generic,reference,_resnet50,_tensorflow,_cpu,_python --adr.compiler.tags=gcc --adr.inference-src.tags=_octoml
