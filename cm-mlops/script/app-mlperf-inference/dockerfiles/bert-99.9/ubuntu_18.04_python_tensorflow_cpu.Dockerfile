...
#RUN cm run script --tags=get,ml-model,language-processing,bert,_tf
#RUN cm run script --tags=get,generic-python-lib,_tokenization
#RUN cm run script --tags=get,generic-python-lib,_protobuf

# Run CM workflow for MLPerf inference
RUN cm run script --tags=app,mlperf,inference,generic,_bert-99.9,_tensorflow,_cpu,_python --adr.compiler.tags=gcc --adr.inference-src.tags=_octoml --fake_run 
RUN cm run script --tags=app,mlperf,inference,generic,_bert-99.9,_tensorflow,_cpu,_python --adr.compiler.tags=gcc --adr.inference-src.tags=_octoml 
