...
def copy_cache_to_remote(i):
    ...
    remote_cm_repos_location = i.get('remote_cm_repos_location', os.path.join("/home", os.environ.get("USERNAME"), "CM", "repos"))
    ...
    for k in sorted(r['list'], key = lambda x: x.meta.get('alias','')):
        ...
        copy_cmd = f"rsync -avz --exclude cm-cached-state.json {path} {remote_host}:{remote_cm_cache_location}"
        print(copy_cmd)
        os.system(copy_cmd)
        ...
        copy_cmd = f"rsync -avz tmp_remote_cached_state.json {remote_host}:{remote_cached_state_file_location}"
        print(copy_cmd)
        os.system(copy_cmd)
